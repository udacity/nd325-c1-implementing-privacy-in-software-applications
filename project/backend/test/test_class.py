...
class TestRegistry:
    ...
    def test_voter_registration(self):
        ...
        for voter in all_voters:
            ...
            assert registry.get_voter_status(voter) == VoterStatus.REGISTERED_NOT_VOTED
    ...
    def test_different_format_national_id(self):
        ...
        assert registry.get_voter_status(voter) == VoterStatus.NOT_REGISTERED
        assert registry.get_voter_status(same_voter) == VoterStatus.NOT_REGISTERED
        assert registry.register_voter(voter), "Couldn't register {0} {1}".format(voter.first_name, voter.last_name)
        ...
        assert registry.get_voter_status(voter) == VoterStatus.REGISTERED_NOT_VOTED
        assert registry.get_voter_status(same_voter) == VoterStatus.REGISTERED_NOT_VOTED
    ...
    def test_de_register_voter(self):
        ...
        assert registry.register_voter(voter), "Couldn't register {0} {1}".format(voter.first_name, voter.last_name)
        assert registry.get_voter_status(voter) == VoterStatus.REGISTERED_NOT_VOTED
        assert registry.de_register_voter(voter), "Couldn't de-register {0} {1}".format(
